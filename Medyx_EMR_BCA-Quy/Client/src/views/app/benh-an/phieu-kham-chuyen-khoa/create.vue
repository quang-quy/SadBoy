<template>
    <el-dialog :visible.sync="dialogVisible" width="800px" top="5vh">
        <template slot="title">
            <div class="px-3">
                <div style="font-size: 16px; font-weight: bold" class="mb-3">
                {{
                    edit 
                    ? 'Cập nhật phiếu khám chuyên khoa' 
                    : 'Thêm mới phiếu khám chuyên khoa'
                }}
                </div>
                <v-progress-linear
                color="primary"
            ...
            <v-row>
                ...
                <v-col cols="3" class="padding-cols">
                    <el-form-item label="3. Ngày chỉ định">
                        <el-date-picker value-format="yyyy-MM-ddTHH:mm:ss" format="dd/MM/yyyy" v-model="form.ngayChiDinh" placeholder="VD: 09/08/2022" size="small" style="width:100%"></el-date-picker>
                    </el-form-item>
                </v-col>
                <v-col cols="3" class="padding-cols"> 
                    <el-form-item label="4. Bác sĩ chỉ định">
                        <base-select-async placeholder="Tìm kiếm theo tên" v-model="form.bacSiChiDinh"
                        :label="(item) => `${item.hoTen}`" keyValue="maNv" :apiFunc="getNhanVien" style="width: 100%"
                        size="small"></base-select-async>
                    </el-form-item>
                </v-col>
                <v-col cols="6" class="padding-cols">
                    <el-form-item label="5. Khoa phòng khám" prop="maKhoa">
                        <base-select-async placeholder="Tìm kiếm theo mã khoa" v-model="form.maKhoa"
                        :label="(item) => `${item.maKhoa} - ${item.tenKhoa}`" keyValue="maKhoa" :apiFunc="getKhoa" style="width: 100%"
                        size="small"></base-select-async>
                    </el-form-item>
                </v-col>
                <v-col cols="12" class="padding-cols">
                    <el-form-item label="6. Kết quả khám và hướng dẫn điều trị">
                        <el-input type="textarea" rows="2" size="small" v-model="form.ketQuaVaHuongDT"></el-input>
                    </el-form-item>
                </v-col>
                <v-col cols="6" class="padding-cols">
                    <el-form-item label="7. Bác sĩ khám">
                        <base-select-async placeholder="Tìm kiếm theo tên" v-model="form.bacSiKham"
                        :label="(item) => `${item.hoTen}`" keyValue="maNv" :apiFunc="getNhanVien" style="width: 100%"
                        size="small"></base-select-async>
                    </el-form-item>
                </v-col>
                <v-col cols="6" class="padding-cols">
                    <el-form-item label="8. Ngày Khám">
                        <el-date-picker value-format="yyyy-MM-ddTHH:mm:ss" format="dd/MM/yyyy" v-model="form.ngayKham" placeholder="VD: 09/08/2022" size="small" style="width:100%"></el-date-picker>
                    </el-form-item>
                </v-col>
                <v-col cols="12">
                        ...
                        <div style="display:flex; justify-content: space-between; align-items: center;">
                            ...
                            <div style="width:720px">
                                ...
                                <div v-else style="height : 160px ;" class="d-flex align-center justify-center">
                                    <div>
                                        <v-icon large color="#8c939d" class="d-flex align-center justify-center mb-2">mdi-file-find</v-icon>
                                        <div style="font-size: 12px;">Không có tệp đính kèm nào</div>
                                    </div>
                                </div>
                            </div> 
                        </div>   
