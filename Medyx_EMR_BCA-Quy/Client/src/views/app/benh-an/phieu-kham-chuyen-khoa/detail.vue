  ...
  <app-wrapper :idba="id">
    <div class="ma-5">
      <div class="d-flex justify-space-between align-flex-end mb-3">
        <div class="d-flex">
          ...
          <div class="ml-3">
            <div style="font-size: 20px; font-weight: bold">
              {{
                  edit
                    ? "Chi tiết phiếu khám chuyên khoa"
        ...
        <div v-if="!idhis">
          <div class="d-flex">
            <v-btn color="primary" @click="submit('form')" small>
        ...
        <v-row class="mt-0">
          ...
          <v-col cols="3" class="padding-cols">
            <el-form-item label="Ngày chỉ định">
              <el-date-picker type="datetime" value-format="yyyy-MM-ddTHH:mm:ss" format="dd/MM/yyyy HH:mm"
                placeholder="VD: 09/08/2022" v-model="form.ngayYlenh" :disabled="true" size="small" style="width: 100%">
              </el-date-picker>
            </el-form-item>
          </v-col>
          <v-col cols="3" class="padding-cols">
            <el-form-item label="Bác sĩ chỉ định">
              <base-select-async placeholder="Tìm kiếm theo tên" v-model="form.bsChiDinh" :disabled="true"
                :label="(item) => `${item.maNv} - ${item.hoTen} - ${item.khoa.tenKhoa}`" keyValue="maNv"
                :apiFunc="getNhanVien" style="width: 100%" size="small"></base-select-async>
            </el-form-item>
          </v-col>
          <v-col cols="3" class="padding-cols">
            <el-form-item label="Khoa phòng khám" prop="maKhoaThucHien">
          ...
          <v-col cols="3" class="padding-cols">
            <el-form-item label="Ngày khám">
              <el-date-picker type="datetime" value-format="yyyy-MM-ddTHH:mm:ss" format="dd/MM/yyyy HH:mm:ss"
                v-model="form.ngayTiepNhan" placeholder="VD: 09/08/2022" size="small" style="width: 100%">
              </el-date-picker>
            </el-form-item>
          </v-col>
          <v-col cols="3" class="padding-cols">
        ...
        <v-row>
          <v-col cols="8">
            <v-row>
              <v-col cols="12" class="padding-cols mt-3 mb-4">
                <label><span style="color: red" class="mr-2">*</span>Kết quả khám và hướng dẫn</label>
          ...
          <v-col cols="4">
            ...
            <!-- <v-row class="px-3 mt-4" v-if="edit">
              ...
              <v-col cols="6" v-for="item in 6" style="text-align: center">
                <el-upload class="avatar-uploader" action="#" :show-file-list="false" :on-success="
...
export default {
  ...
  methods: {
    goBack() {
      window.location = "/HSBADS/PhieuKhamChuyenKhoa/" + this.id;
    },
    clickUpload() {
    ...
    async getDSDichVu(params) {
      return await getDichVu({ ...params, MaChungLoai: 1 });
    },
    submit(formName) {
    ...
    async removeFile(index, item) {
      if (!this.edit) {
        ...
        await deleteFile(item.idba, item.stt);
        this.getDsFile();
      }
    },
    ...
    async getData() {
      if (this.stt) {
        ...
        this.form.ketLuan = data.data.benhAnClsKq.ketLuan;
      }
    },
    ...
    async getImange() {
      try {
        await getImangeBaCls(this.id, this.stt)
      } catch (error) {
        console.log(error)
      }
    },
