<template>
  <div class="align-center pa-2 ml-1">
    <div class="d-flex justify-space-between">
      <div class="d-flex">
        <v-icon dark color="primary">mdi-file</v-icon>
        <div class="text-h6 ml-3 align-center">8. Danh sách file đính kèm</div>
      </div>
    </div>
    <v-progress-linear
      :active="!show"
...
export default {
  ...
  methods: {
    ...
    async handleGetListFile() {
      ...
      this.show = false;
      this.fileList = [];
      var { data } = await getListFile({
        loaiTaiLieu: 6,
        huy: false,
        idba: this.params.idba,
        sttDv: this.dataDetail.stt,
      });
      let tempt = data;
    ...
    async handleRemove(file) {
      ...
      if (res) {
        const response = await deleteFile(file.idba, file.stt);
    ...
    async handleUpload(file) {
      ...
      formData.append("loaiTaiLieu", 6);
      formData.append("idba", this.params.idba);
      formData.append("sttDv", this.dataDetail.stt);
      try {
