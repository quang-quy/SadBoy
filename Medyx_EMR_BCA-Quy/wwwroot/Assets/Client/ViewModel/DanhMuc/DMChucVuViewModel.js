...
var DMChucVuViewModel = function () {
    //khởi tạo tham số
    ...
    //mảng lưu danh mục
    self.DMChucVus = ko.observableArray();
    ...
    self.columns = ko.observableArray([
        ...
        { property: "MaCV", header: "Mã chức vụ", type: "string", state: ko.observable(""), width: '10%', visible: ko.observable(true), field: '', search: 'string' },
        { property: "TenCV", header: "Tên chức vụ(*)", type: "string", state: ko.observable(""), width: '32%', visible: ko.observable(true), field: 'input', search: 'string' },
    ...
    self.stringSort = function (column) { // Pass in the column object
        self.DMChucVus(self.DMChucVus().sort(function (a, b) {
    ...
    self.displayMode = function (DMChucVu) {
        return DMChucVu.Edit() ? 'edit-template' : 'read-template';
    }
    //sự kiện nhấn phím khi đang sửa thông tin bản ghi
    self.updateEnterInput = function (DMChucVu, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 13) {
            DMChucVu.TenCV = $('#txtTenCV_Edit').val();
            self.done(DMChucVu);
            return false;
        }
        return true;
    ...
    self.updateEnterHuy = function (DMChucVu, event) {
        var keyCode = (event.which ? event.which : event.keyCode);
        if (keyCode === 32) {
            if ($('#chkHuy_edit').is(":checked")) {
                DMChucVu.Huy = !DMChucVu.Huy;
    ...
    //sửa thông tin
    self.edit = function (DMChucVu) {
        if ($('#txtQuyenXem').val() == 'true') {
            if (temp != undefined) {
                self.cancel(temp);
            }
            temp = DMChucVu;
            tempTenCV = DMChucVu.TenCV;
            tempHuy = DMChucVu.Huy;
            self.AddNew(false);
            DMChucVu.Edit(true);
            //$('#' + self.selectedItem().MaCV).focus();
            $('#txtTenCV_Edit').focus();
    ...
    self.done = function (DMChucVu) {
        if (DMChucVu.TenCV.trim() == "") {
            DMChucVu.Edit(false);
    ...
    self.subCancel = function (DMChucVu) {
        for (var i = 0; i < self.DMChucVus().length; i++) {
            if (self.DMChucVus()[i].MaCV == DMChucVu.MaCV) {
                self.DMChucVus()[i].TenCV = tempTenCV;
                self.DMChucVus()[i].Huy = tempHuy;
            }
        }
        DMChucVu.Edit(false);
        //$("tbody").css("overflow-y", "auto");
    ...
    self.cancel = function (DMChucVu) {
        self.subCancel(DMChucVu);
        temp = null;
        tempTenCV = "";
        tempHuy = false;
        var item = self.DMChucVus()[0];
        if (self.AddNew()) {
            self.DMChucVus.remove(DMChucVu);
            self.AddNew(false)
        }
    }
    //xóa bản ghi
    self.delete = function (DMChucVu) {
        if ($('#txtQuyenXem').val() == 'true') {
            if (confirm('Bạn chắc chắn muốn xóa chức vụ "' + DMChucVu.MaCV + ' - ' + DMChucVu.TenCV + '" ?')) {
                DMChucVu.Huy = true;
                self.deleteChucVu(DMChucVu);
            }
        }
    }
    //hàm xác định bản ghi đang được chọn
    self.selectItem = function (DMChucVu) {
        self.selectedItem(DMChucVu);
    };
    //searchResults = dmTinhs --- selectResult = selectItem --- selectedResult = selectedItem
    //move row up and down in table
    self.selectPrevious = function () {
        //var index = self.DMChucVus().indexOf(self.selectedItem()) - 1;
        var index = 0;
        if (self.selectedItem() != undefined) {
            if (self.selectedItem().STT > 1 && self.selectedItem().STT % self.pageSize() == 0) {
                ...
                index = self.DMChucVus().indexOf(self.selectedItem()) - 1;
        }
        if (index < 0) index = 0;
        self.selectedItem(self.DMChucVus()[index]);
        Extention.selectHeaderRow($('#' + self.selectedItem().MaCV), 31);
        $('#' + self.selectedItem().MaCV).focus();
    };
    self.selectNext = function () {
        //var index = self.DMChucVus().indexOf(self.selectedItem()) + 1;
        var index = 0;
        if (self.selectedItem() != undefined) { index = self.DMChucVus().indexOf(self.selectedItem()) + 1; }
        if (index >= self.DMChucVus().length) index = self.DMChucVus().length - 1;
        self.selectedItem(self.DMChucVus()[index]);
        Extention.selectHeaderRow($('#' + self.selectedItem().MaCV), 31);
        $('#' + self.selectedItem().MaCV).focus();
    };
    ...
    self.upAndDown = function (data, e) {
        ...
        if (e.keyCode == 38) {
            ...
            if (this.MaCV != self.selectedItem().MaCV) {
                var index = self.DMChucVus().indexOf(self.selectedItem()) + 1;
                if (index >= self.DMChucVus().length) $('#table').focusout();
                else $('#' + self.selectedItem().MaCV).focus();
            }
            return false;
        }
        else return true;
    ...
    $(document).ready(function () {
        ...
        if ($('#txtQuyenXem').val() == 'true') {
            ...
            $('#txtiTenCV').attr('disabled', 'disabled');
        }
    ...
    self.loadData = function () {
        ...
        $.getJSON('/DMChucVu/LoadData', data1, function (data) {
            self.tongSoBanGhi(data.totalCounts);
            self.tongSoTrang = data.totalPages;
            self.DMChucVus(ko.utils.arrayMap(data.items, function (DMChucVu) {
                STT += 1;
                var objDMChucVu = {
                    STT: STT,
                    Edit: ko.observable(false),
                    MaCV: DMChucVu.maCV,
                    TenCV: DMChucVu.tenCV,
                    MaMay: (DMChucVu.maMay == undefined) ? "" : DMChucVu.maMay,
                    Huy: DMChucVu.huy,
                    NguoiSD: (DMChucVu.nguoiSD == undefined) ? "" : DMChucVu.nguoiSD,
                    NgaySD: Extention.ConvertSQLDateTimeToStringFormat(DMChucVu.ngaySD)
                }
                return objDMChucVu;

                if (DMChucVu.Huy == false) {
                    $('#chkHuy_read').checked(true);
                }
            }));
            if (self.DMChucVus().length != 0) {
                if (self.LoadLanDau())
                    self.paging();
                if (self.selectedItem() != undefined) {
                    self.selectedItem().Edit(false);
                    self.selectItem(self.selectedItem());
                    $('#' + self.selectedItem().MaCV).focus().addClass('danger');
                }
            }
            else {
    ...
    self.init();
    //thêm mới bản ghi
    self.insert = function (DMChucVu) {
        if ($('#txtQuyenXem').val() == 'true') {
            self.checkAdd(true);
            if ($('#txtiTenCV').val().trim() != '') {
                $.ajax({
                    url: '/DMChucVu/CreateChucVu',
                    type: 'POST',
                    data: { tenCV: $('#txtiTenCV').val().trim() },
                ...
                alert('Thêm mới không thành công! Kiểm tra lại trường nhập tên chức vụ');
                Extention.errorBorder('txtiTenCV');
                self.cancelInsert();
                $('#txtiTenCV').focus();
            }
        }
    ...
    //xóa bản ghi
    self.deleteChucVu = function (DMChucVu) {
        $.ajax({
            url: '/DMChucVu/DeleteChucVu',
            type: 'POST',
            data: { maCV: DMChucVu.MaCV },
            success: function (response) {
                if (response != null && response.success == false)
                    alert(response.message);
                else {
    ...
    self.Export = function () {
        if (self.tongSoBanGhi() <= 1048570) {
            if (self.colExport().length > 0) {
                ...
                window.open('/DMChucVu/ExportExcel?obj=' + JSON.stringify(obj));
                //$.ajax({
                //    url: '/DMChucVu/ExportDMChucVu',
                //    type: 'POST',
    ...
    self.updateDMChucVu = function (DMChucVu) {
        $.ajax({
            url: '/DMChucVu/Modify',
            type: 'POST',
            data: { chucVu: DMChucVu },
            success: function (response) {
                if (response != null && response.success == false)
                    alert(response.message);
                else {
...
ko.applyBindings(new DMChucVuViewModel());